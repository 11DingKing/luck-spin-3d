...
const AudioManager = {
  ...
  startTickLoop(interval = 80) {
    this.stopTickLoop();
    this.tickInterval = setInterval(() => {
      if (LotteryStorage.getSettings().soundEnabled) {
        this.playTick();
      }
    }, interval);
  },
  ...
  playCelebration() {
    ...
    for (let i = 0; i < 8; i++) {
      ...
      const tinkleTime = startTime + 0.5 + Math.random() * 0.8;
      gain.gain.setValueAtTime(0, tinkleTime);
      gain.gain.linearRampToValueAtTime(0.08, tinkleTime + 0.01);
      gain.gain.exponentialRampToValueAtTime(0.001, tinkleTime + 0.2);

      osc.start(tinkleTime);
      osc.stop(tinkleTime + 0.2);
    }
  },
};
