...
function renderParticipantList() {
  try {
    ...
    container.innerHTML = availableList
      .map(
        (p) => `
    ...
    Logger.debug("Participant list rendered", {
...
function selectPrize(prizeId) {
  try {
    ...
    if (prizeInfoEl)
      prizeInfoEl.textContent = `剩余 ${prize.count - drawnCount} 个名额`;
...
async function toggleLottery() {
  try {
    ...
    if (!isLotteryRunning) {
      ...
      const settings = LotteryStorage.getSettings();
      const started = Lottery.start(
        currentPrizeId,
        availableParticipants,
        settings.lotterySpeed,
      );
      ...
      if (started) {
        ...
        const intervalTime =
          settings.lotterySpeed === "fast"
            ? 100
            : settings.lotterySpeed === "slow"
              ? 150
              : 120;
        AudioManager.startTickLoop(intervalTime);
      } else {
      ...
      if (winner) {
        ...
        try {
          ...
          // 检查奖品是否抽完
          const updatedPrize = LotteryStorage.getPrizes().find(
            (p) => p.id === currentPrizeId,
          );
          const prizeInfoEl = document.getElementById("currentPrizeInfo");
          const prizeNameEl = document.getElementById("currentPrizeName");
    ...
    Logger.error("Lottery toggle failed", {
      error: error.message,
      stack: error.stack,
    });
  ...
  try {
    ...
    for (let i = 0; i < num; i++) {
      ...
      if (Math.random() > 0.3) {
        phone =
          "1" +
          ["3", "5", "7", "8", "9"][Math.floor(Math.random() * 5)] +
    ...
    try {
      ...
      for (let i = 1; i < jsonData.length; i++) {
        ...
        participants.push({
          ...
          department:
            deptIndex !== -1 ? String(row[deptIndex] || "").trim() : "",
        });
      }
...
async function clearParticipants() {
  try {
    ...
    const confirmed = await showConfirm(
      `确定要清空所有 <strong>${count}</strong> 名参与者吗？`,
      "清空名单",
    );
    if (!confirmed) {
      return;
...
function exportWinnerList() {
  try {
    ...
    Logger.action("EXPORT_WINNERS", {
      fileName,
      recordCount: exportData.length - 1,
    });
  } catch (error) {
