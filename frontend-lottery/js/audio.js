...
const AudioManager = {
    ...
    init() {
        try {
            ...
            Logger.error('Failed to initialize AudioContext', { error: error.message });
        }
    },
    ...
    playCelebration() {
        ...
        notes.forEach((freq, index) =&gt; {
            ...
            gainNode.gain.exponentialRampToValueAtTime(0.001, startTime + index * 0.15 + 0.5);
        ...
        for (let i = 0; i &lt; 3; i++) {
            ...
            noiseOsc.frequency.setValueAtTime(200 + Math.random() * 1000, startTime + 0.2 + i * 0.2);
            
            filter.type = 'lowpass';
            filter.frequency.setValueAtTime(5000, startTime + 0.2 + i * 0.2);

            noiseGain.gain.setValueAtTime(0, startTime + 0.2 + i * 0.2);
            noiseGain.gain.linearRampToValueAtTime(0.1, startTime + 0.2 + i * 0.2 + 0.02);
            noiseGain.gain.exponentialRampToValueAtTime(0.001, startTime + 0.2 + i * 0.2 + 0.3);
